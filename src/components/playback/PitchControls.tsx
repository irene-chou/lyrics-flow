...
export function PitchControls() {
  ...
  const pitchSemitones = usePlaybackStore((s) => s.pitchSemitones)
  ...
  function adjustPitch(delta: number) {
    const current = usePlaybackStore.getState().pitchSemitones
    const newPitch = Math.round((current + delta) * 2) / 2 // round to 0.5
    const clamped = Math.max(-12, Math.min(12, newPitch))
    usePlaybackStore.getState().setPitchSemitones(clamped)
  }

  function resetPitch() {
    usePlaybackStore.getState().setPitchSemitones(0)
  }
  ...
  return (
    <div style={{ marginTop: '4px' }}>
      ...
      <div className="flex items-center justify-center" style={{ gap: '6px' }}>
        ...
        >
          {pitchSemitones >= 0 ? '+' : ''}
          {pitchSemitones % 1 === 0 ? pitchSemitones.toFixed(0) : pitchSemitones.toFixed(1)}
        </span>
        <button className={CONTROL_BTN_CLASS} onClick={() => adjustPitch(0.5)} style={OFFSET_BTN_STYLE}>
          +½
        </button>
